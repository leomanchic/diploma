...
измерений. S7B находится на cross-platform corrective gate: compatibility
проверяется по финальному constrained solution, а projected-KKT должен быть
dimensionless Newton-correction metric, инвариантной к rigid transforms и
масштабу сцены. S7C остаётся `Planned (Next)` и должен позднее реализовать
